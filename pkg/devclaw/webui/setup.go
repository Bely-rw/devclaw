...
func (s *Server) handleSetupFinalize(w http.ResponseWriter, r *http.Request) {
	...
	if s.onVaultInit != nil {
		...
		if len(secrets) > 0 {
			if err := s.onVaultInit(vaultPassword, secrets); err != nil {
				...
				// Fallback: write API key to .env so the system still works.
				envContent := "# DevClaw — generated by setup wizard\n" +
					"# FALLBACK: vault creation failed, secrets in plain text.\n"
				if setup.APIKey != "" {
					envContent += "DEVCLAW_API_KEY=" + setup.APIKey + "\n"
				}
	...
	} else if setup.APIKey != "" {
		// No vault callback available — fallback to .env.
		s.logger.Warn("no vault init callback — falling back to .env (less secure)")
		envContent := "# DevClaw — generated by setup wizard\n" +
			"# WARNING: vault unavailable, secrets in plain text.\n" +
			"DEVCLAW_API_KEY=" + setup.APIKey + "\n"
		if err := os.WriteFile(".env", []byte(envContent), 0o600); err != nil {
			s.logger.Error("failed to write .env", "error", err)
		}
	}

	// .env only holds the vault password for non-interactive unlock (PM2,
	// Docker, systemd). Secrets themselves are encrypted in the vault.
	envContent := "# DevClaw — generated by setup wizard\n" +
		"# Only the vault password is stored here. All other secrets\n" +
		"# (API keys, tokens) are encrypted in .devclaw.vault.\n" +
